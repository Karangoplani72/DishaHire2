...
const EnquiryModal: React.FC<Props> = ({ type, onClose, initialMessage = '', initialSubject = 'General Inquiry' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.message || 'Transmission failed. Please check your connectivity.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4">
      ...
      <div className="bg-white w-full max-w-xl rounded-[2rem] overflow-hidden relative z-10 shadow-2xl flex flex-col max-h-[95vh] sm:max-h-[90vh]">
        ...
        <div className="bg-brand-dark p-6 sm:p-8 text-white relative flex-shrink-0">
          ...
          <div className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-gold mb-1">Secure Submission Portal</div>
        ...
        <form className="p-6 sm:p-10 space-y-5 overflow-y-auto" onSubmit={handleSubmit}>
          ...
          {type === EnquiryType.EMPLOYER ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Hiring Priority</label>
          ...
          <div className="space-y-1">
            ...
            <textarea defaultValue={initialMessage} name="message" rows={3} className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:bg-white focus:border-brand-gold/30 outline-none transition-all resize-none text-sm leading-relaxed" placeholder="Describe your requirements..."></textarea>
          </div>

          {type === EnquiryType.CANDIDATE && (
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Upload Professional CV (PDF Only)</label>
              <div className="relative border-2 border-dashed border-gray-200 bg-gray-50 rounded-xl p-6 text-center hover:border-brand-gold/40 hover:bg-white transition-all cursor-pointer">
                <input type="file" accept=".pdf" onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                {file ? <span className="text-brand-dark font-bold text-xs">{file.name}</span> : <span className="text-gray-400 text-xs">Drop PDF file here or click to browse</span>}
              </div>
              {fileError && <p className="text-red-500 text-[10px] font-black text-center">{fileError}</p>}
            </div>
          )}
          ...
          <button disabled={loading} className="w-full bg-brand-dark text-white py-4 rounded-xl font-bold text-base flex items-center justify-center gap-3 shadow-xl hover:bg-brand-accent transition-all transform hover:-translate-y-1 mt-4">
            {loading ? <Loader2 className="animate-spin" /> : <>Send Secure Inquiry <Send size={18} /></>}
          </button>
        </form>
      </div>
    </div>
